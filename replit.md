...
Per `exports/ARK_PDD_MVP_Spartan.md`, the core deployed surface is the 7 CLASS A features (Identity, Resume Analyzer, CCGE Arena, SPHINX MVP, Bonsai onboarding, Billing FREE+PRO, GDPR). CLASS C surfaces are behind feature flags so any one can be re-gated per trigger family (≥100 listings, first SCHOOL_STUDENT licence, etc.) via env without redeploying. As of the deferred-module activation **plus the two-stub completion, every flag now defaults ON** — including the two formerly half-built stubs (`enterpriseDashboard`, `assessmentEmail`), which are now fully implemented (see lists below).

- **Single source of truth**: `shared/featureFlags.ts::FEATURES` — 24 flag keys, all defaulting `true`. Re-gate any surface with the env overlay (`FEATURE_<SNAKE_CASE>=false`) rather than editing defaults.
...
- `/ark-resume` — **ARK RESUME**: single-page ATS-optimized resume artifact fusing lifted static resume + verified-card "living" layer + third-party confirmations; JST top-right, ATS score breakdown, selectable-text PDF export. Pro+ AND ≥1 Silver+ verification gated (locked state otherwise).
...
**ARK RESUME (Task #59 — `arkResume` flag)**
- `GET /api/ark-resume` — aggregated resume payload (identity, JST, work history with per-company mapped cards + confirmation, education, certs, global verified deck, ATS score + breakdown, eligibility). 403 with `eligibility` body when Pro+ / Silver+ gate fails.
- `POST /api/ark-resume/headshot` (`{ dataUrl }`) · `DELETE /api/ark-resume/headshot`
- `GET /api/confirmations` (own confirmations) · `POST /api/confirmations` (confirmer-gated: admin OR instructor role OR owns the cited training provider)
