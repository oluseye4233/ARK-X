...
Per `exports/ARK_PDD_MVP_Spartan.md`, the core deployed surface is the 7 CLASS A features (Identity, Resume Analyzer, CCGE Arena, SPHINX MVP, Bonsai onboarding, Billing FREE+PRO, GDPR). CLASS C surfaces are behind feature flags so the gate can lift per trigger family (≥100 listings, first SCHOOL_STUDENT licence, etc.) without redeploying. As of the deferred-module activation, **only the two half-built stubs (`enterpriseDashboard`, `assessmentEmail`) remain flagged off** — every other CLASS C module is now ON (see lists below).

- **Single source of truth**: `shared/featureFlags.ts::FEATURES` — 24 flag keys; only `enterpriseDashboard` and `assessmentEmail` default `false` (the two half-built stubs), all others default `true`.
...
**Currently flagged OFF (Stage 1)**: `enterpriseDashboard` (half-built stub — only `/api/departments` exists; its live analytics overlap the `institutionWorkforce` aggregation) and `assessmentEmail` (half-built stub — `POST /api/notifications/assessment-summary` returns a preview payload and does not dispatch mail). Both are tracked as a follow-on "finish the stubs" task. Phase J implementation code is preserved verbatim — no deletion.
