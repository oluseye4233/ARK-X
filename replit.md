...
Per `exports/ARK_PDD_Current_MVP_Spartan.md` (updated 02 JUL 2026), **all 24 feature flags default ON** — zero deferrals. Any surface can be re-gated with the env overlay (`FEATURE_<SNAKE_CASE>=false`) without redeploying.

- **Single source of truth**: `shared/featureFlags.ts::FEATURES` — 24 flag keys, all defaulting `true`.
- **Server gate**: `server/featureFlags.ts::requireFeature(key)` middleware returns **404** (not 403 — flagged surfaces are indistinguishable from unimplemented routes).
...
Flag families (full per-flag implementation notes: `CHANGELOG.md → Stage 1 — Flag Activation Detail`):
- **Core seven**: `investorDemo` (`/demo` + `/demo-tour` — home CTAs depend on it; landing page shows ONLY the F1000 QR promo as its scarcity badge), `executiveReport` (`/report`, Pro+), `cardVerification` (verification quests + DATA-pillar evidence + Name-Job-Role guide, +40 ARK/day cap), `trainingProviders`, `f1000Promo` (`/f1000` + landing QR; 1,000 single-use codes, price-capped upgrade), `arkResume` (`/ark-resume`, Pro+ AND ≥1 Silver+ verification), `matchmaking` (ARK Talent Exchange — verified-evidence-only scoring).
- **Activated fifteen** (flag flips of fully-built modules): `cohorts`, `guinPublic`, `notifications`, `claudeNarrative`, `subscriptionCancel`, `institutionWorkforce`, `corporateMarketplace`, `forgeLabDocx`, `drm`, `customScenarios`, `adminCcgeImport`, `contextCraftPage`, `revenueGuardrail`, `bookCompanion`, `sphinxAdvanced`.
- **Completed two** (former stubs, fully implemented): `enterpriseDashboard` — real `/enterprise` Workforce Intelligence (live aggregation via `storage.getWorkforceIntelligence`, AND-stacked tenure/location filters, bounded department drill-down with search + paging, org-wide staff search, upskill nudge with in-app SSE notification + best-effort email + 7-day cooldown; `migrations/0022_staff_nudge.sql`); `assessmentEmail` — real plain-text summary email via the shared Gmail transport, recipient always the authenticated user's own address, truthful 502 on failure.
...
- **AI**: Multi-provider, LLM-resilient. Anthropic Claude (default: Haiku for KCSE, Sonnet for narrative + scenario gen), OpenAI (gpt-5-mini, gpt-5.4), and Gemini (2.5-flash, 2.5-pro) via the three Replit AI integrations. Roster + pricing in `shared/schema.ts::SELECTABLE_AI_MODELS`; provider layer in `server/ai/providers.ts` (`resolveModelChain` = preferred → task default → Haiku anchor → cross-provider resilience tail (any available+plan-allowed model, economy first); throws 503 only when nothing is usable; per-call fallback via `generateWithChain`). Internal utility calls without user/plan context (`arkResume` card-mapping, `profileExtract` bio) use `resolveUtilityChain` (availability-only, economy tier) and degrade gracefully when empty. AI response caches are keyed on the resolved primary model (`chain[0]`), never a hardcoded model literal. Users pick a model on `/profile` (AI Engine card); stored in `users.preferredAiModel`. Plan policy: FREE = economy models only (`AI_ECONOMY_MODELS`), paid tiers = full roster (`AI_TIER_MODEL_POLICY`). Endpoints: `GET /api/ai/models`, `PUT /api/ai/model-preference`. Guardrail order unchanged: budget gates → model resolution → call → `logUsage` with the actually-used model. See `CHANGELOG.md → Phase F`.
...
- `/enterprise` — Workforce Intelligence (live aggregation, dimension filters, department drill-down, org-wide staff search, upskill nudge)
