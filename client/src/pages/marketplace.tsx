...
function SynthesisTab({ listingId }: { listingId: string }) {
  const [data, setData] = useState<{ count: number; recent: any[] } | null>(null);
  useEffect(() => { api.getListingSyntheses(listingId).then(setData).catch(() => null); }, [listingId]);
  return (
    <div className="glass-card p-6 rounded-xl border border-white/10 space-y-4" data-testid="panel-synthesis">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Sparkles className="h-5 w-5 text-primary" />
          <h3 className="font-display font-bold text-base text-white tracking-wider uppercase">Synthesis Activity</h3>
        </div>
        <Link href="/marketplace/synthesis" className="px-3 py-1.5 rounded font-mono text-[10px] uppercase tracking-wider bg-primary/10 text-primary border border-primary/30 hover:bg-primary/15" data-testid="link-open-synthesis">
          Combine cards →
        </Link>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="p-4 rounded-lg border border-white/10 bg-white/5">
          <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground mb-1">Used in</div>
          <div className="font-mono text-2xl font-bold text-primary" data-testid="text-synthesis-count">
            {data ? data.count : "—"} <span className="text-sm text-muted-foreground font-normal">syntheses</span>
          </div>
        </div>
        <div className="p-4 rounded-lg border border-white/10 bg-white/5">
          <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground mb-1">Avg ZPOS reduction</div>
          <div className="font-mono text-2xl font-bold text-secondary" data-testid="text-avg-reduction">
      ...
      {data && data.recent.length > 0 && (
        <div className="space-y-2">
          <div className="text-[10px] uppercase font-mono tracking-widest text-muted-foreground">Recent finalized syntheses</div>
          {data.recent.map((r) => (
            <div key={r.sessionId} className="flex items-center justify-between p-2.5 rounded border border-white/5 bg-white/5" data-testid={`row-synthesis-${r.sessionId}`}>
              <span className="font-mono text-[10px] text-muted-foreground">{new Date(r.createdAt).toLocaleDateString()}</span>
              <span className="font-mono text-xs text-primary">{r.zposMethod}</span>
              <span className="font-mono text-xs text-secondary">−{Math.round(r.reductionPct)}%</span>
            </div>
          ))}
        </div>
      )}
      <p className="font-mono text-[11px] text-muted-foreground/80 border-t border-white/5 pt-3">
        When buyers combine this SPC with others, you earn a pro-rata share of the synthesis revenue
        (70% creator pool, weighted by source price).
      </p>
    </div>
  );
}
...
type SynthSession = {
  ...
  status: string;
  splitPreview: { creatorId: string; sourceListingId: string; sourcePriceCredits: number; weight: number; creditedAmount: number }[];
...
function SynthesisPage() {
  ...
  useEffect(() => {
    api.getSpcListings().then(setListings).catch((e) => setError(e?.message));
    if (user) api.getCredits(user.id).then(setCredits).catch(() => null);
  }, [user?.id]);

  const toggle = (id: string) => {
    setSelected((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : (prev.length >= 7 ? prev : [...prev, id]));
    setSession(null); setOutcome(null); setError(null);
  };
  ...
  const preview = async () => {
    ...
    try {
      ...
      setSession(s);
    } catch (e: any) { setError(e?.message || "Synthesis preview failed."); }
  ...
  const finalize = async () => {
    ...
    try {
      ...
      setOutcome(r); setSession(r.session);
      const refreshed = await api.getCredits(user.id); setCredits(refreshed);
    } catch (e: any) { setError(e?.message || "Finalize failed."); }
  ...
  const filtered = useMemo(() => {
    ...
    const q = filter.trim().toLowerCase();
    const own = user?.id;
    return listings.filter((l) => l.creatorId !== own && (!q || l.title.toLowerCase().includes(q) || l.description.toLowerCase().includes(q)));
  }, [listings, filter, user?.id]);

  const insufficient = !!(credits && session && credits.balance < session.totalCreditPrice);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-display font-bold text-primary tracking-widest uppercase" data-testid="text-synthesis-title">
              Synthesis Engine
            </h1>
            <p className="text-muted-foreground font-mono text-xs">
              Combine 2–7 SPCs into one ZPOS-compressed prompt. Royalties split 70/30 by source price weight.
            </p>
          </div>
        </div>
        <Link href="/marketplace" className="font-mono text-xs uppercase tracking-wider text-muted-foreground hover:text-primary inline-flex items-center gap-2" data-testid="link-back-synthesis">
          <ArrowLeft className="h-4 w-4" /> Back
        </Link>
      </div>

      {!user && (
        <div className="glass-card p-6 rounded-xl border border-amber-400/30 bg-amber-400/5 text-center font-mono text-sm text-amber-400" data-testid="text-synthesis-login">
          Please <Link href="/login" className="underline">log in</Link> to run a synthesis.
        </div>
      )}

      <div className="glass-card p-4 rounded-xl border border-primary/20 space-y-3">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <span className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground" data-testid="text-cart-count">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => { setSelected([]); setSession(null); setOutcome(null); }}
              disabled={selected.length === 0}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="font-mono text-[10px] text-amber-400">{l.priceCredits} cr</span>
                  </div>
                  <p className="font-mono text-xs text-white font-bold truncate">{l.title}</p>
                  <p className="font-mono text-[10px] text-muted-foreground truncate">{l.pillar}</p>
                </button>
              );
            })}
          </div>
      ...
      {error && (
        <div className="p-3 rounded-lg border border-destructive/30 bg-destructive/5 font-mono text-sm text-destructive flex items-center gap-2" data-testid="text-synthesis-error">
          <AlertTriangle className="h-4 w-4" /> {error}
        </div>
      )}

      {session && (
        <div className="glass-card p-5 rounded-xl border border-secondary/30 bg-secondary/5 space-y-4" data-testid="panel-synthesis-preview">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="p-3 rounded border border-white/10 bg-white/5">
              <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground">ZPOS</div>
              <div className="font-mono text-base font-bold text-primary" data-testid="text-zpos-method">{session.zposMethod}</div>
            </div>
            <div className="p-3 rounded border border-white/10 bg-white/5">
              <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground">Pre tokens</div>
              <div className="font-mono text-base font-bold text-white" data-testid="text-pre-tokens">{session.preTokens}</div>
            </div>
            <div className="p-3 rounded border border-white/10 bg-white/5">
              <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground">Post tokens</div>
              <div className="font-mono text-base font-bold text-white" data-testid="text-post-tokens">{session.postTokens}</div>
            </div>
            <div className="p-3 rounded border border-white/10 bg-white/5">
              <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground">Reduction</div>
              <div className="font-mono text-base font-bold text-secondary" data-testid="text-reduction-pct">−{Math.round(session.reductionPct)}%</div>
            </div>
            <div className="p-3 rounded border border-white/10 bg-white/5">
              <div className="text-[9px] uppercase font-mono tracking-widest text-muted-foreground">Semantic</div>
              <div className="font-mono text-base font-bold text-amber-400" data-testid="text-semantic">{session.semanticPreservation}%</div>
            </div>
          </div>

          <div>
            <div className="text-[10px] uppercase font-mono tracking-widest text-muted-foreground mb-2">Royalty split (locked at preview)</div>
            <div className="space-y-1">
              {session.splitPreview.map((s) => (
                <div key={s.sourceListingId} className="flex items-center justify-between p-2 rounded border border-white/5 bg-white/5 font-mono text-xs" data-testid={`row-split-${s.sourceListingId}`}>
                  <span className="text-muted-foreground truncate flex-1">{s.sourceListingId.slice(0, 12)}…</span>
                  <span className="text-white mx-3">{Math.round(s.weight * 100)}%</span>
          ...
          {session.status === "preview" && (
            <div className="space-y-2">
              <div className="text-[10px] uppercase font-mono tracking-widest text-muted-foreground">Compressed prompt preview</div>
              <pre className="glass-card p-3 rounded border border-primary/20 text-[11px] text-white/80 font-mono whitespace-pre-wrap max-h-40 overflow-y-auto" data-testid="text-combined-preview">
                {(session as any).previewSnippet || ""}
                {(session as any).previewTruncated ? " …" : ""}
              </pre>
              <div className="p-2.5 rounded border border-amber-400/30 bg-amber-400/5 text-amber-400 font-mono text-[11px] flex items-center gap-2" data-testid="text-locked-preview">
                <Lock className="h-3.5 w-3.5" />
                Full compressed body locked — finalize for {session.totalCreditPrice} cr to unlock.
              </div>
            </div>
          )}
          ...
          {!outcome && session.status === "preview" && (
            ...
            >
              {finalizing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Coins className="h-4 w-4" />}
              {insufficient
          ...
          {outcome && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-3 rounded-lg border border-secondary/30 bg-secondary/5 font-mono text-sm text-secondary flex items-center gap-2" data-testid="text-synthesis-success">
              <CheckCircle2 className="h-4 w-4" /> Synthesis finalized — combined prompt unlocked. Balance:
              <span className="text-white">{formatPriceDual(outcome.buyerBalance)}</span>.
            </motion.div>
          )}
        </div>
      )}
